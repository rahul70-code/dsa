...
public:
    ...
    void backtrack(vector<int> &curr, int target, int sum, vector<int> &candidates, int curInd, int n){
        if(target == sum){
        ...
        for(int i = curInd; i < n; i++){
            ...
            backtrack(curr, target, sum, candidates, i+1, n);
            sum -= candidates[i];
            curr.pop_back();
        }
    ...
    vector<vector<int>> combinationSum2(vector<int>& candidates, int target) {
        ...
        backtrack(curr, target, 0, candidates, 0, n);
        return result;
    }
};